...
echo "   ${bold}    --server${normal}    - Starts local server or sets remote IP for client."
echo "   ${bold}    --size${normal}      - Message size in bytes (between ${bold}${SOCKPERF_MIN}${normal} and ${bold}${safe_msg_size},${normal} default is 1024)."
echo ""
