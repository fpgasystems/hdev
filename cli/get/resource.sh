...
PLATFORMINFO_PARAMETER="Resource Availability:"
...
if [ "$flags" = "" ]; then
    ...
    for device_index in $(seq 1 $MAX_DEVICES); do 
        ...
        #get parameter
        resource=$(get_platforminfo_parameter "$PLATFORMINFO_PARAMETER" "$platform" "$PLATFORMINFO_LIST")

        #print        
        if [ -n "$platform" ]; then
            echo "$device_index: $resource"
        fi
    done
    echo ""
else
