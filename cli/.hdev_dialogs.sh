...
CHECK_ON_FILENAME_ERR_MSG="Please, choose a valid filename."
...
CHECK_ON_VITIS_HLS_ERR_MSG="Please, choose a valid Vitis HLS version."
...
partition_check() {
  ...
  #get partitions
  MAX_PARTITIONS=$($CLI_PATH/get/partitions --device $device_index --type "primary" | sed -n 's/.*\([0-9]\)]/\1/p')
...
vitis_hls_check() {
  local VITIS_HLS_PATH=$1
  local vivado_version=$2
  if [ -z "$vivado_version" ] || [ ! -d $VITIS_HLS_PATH/$vivado_version ]; then
    echo ""
    echo $CHECK_ON_VITIS_HLS_ERR_MSG
    echo ""
    exit 1
  fi
}
