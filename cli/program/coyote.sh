...
#usage:       $CLI_PATH/hdev program coyote --commit $commit_name --device $device_index --project $project_name --version $vivado_version --remote $deploy_option 
#example: /opt/hdev/cli/hdev program coyote --commit      8077751 --device             1 --project   hello_world --version          2022.1 --remote              0 
...
#get networking addresses
ip_hex=$($CLI_PATH/common/get_interface "$DEVICES_LIST_FPGA" $device_index $NETWORKING_PORT_INDEX ip hex)
mac_hex=$($CLI_PATH/common/get_interface "$DEVICES_LIST_FPGA" $device_index $NETWORKING_PORT_INDEX mac hex)
...
if ! lsmod | grep -q ${driver_name_base%.ko}; then
    eval "$CLI_PATH/program/driver -i $DIR/$DRIVER_NAME -p ip_addr=$ip_hex,mac_addr=$mac_hex --remote 0"
fi
...
programming_string="$CLI_PATH/program/coyote --commit $commit_name --device $device_index --project $project_name --version $vivado_version --remote 0"
